...
int AVL(struct wezel *&korzen, int klucz)
{
	...
	if ( biezacy->lewy && !biezacy->prawy )
	{
		biezacy->wspolczynnik_wywazenia = -1;
		biezacy = ojciec ( korzen, biezacy->klucz );

		if ( biezacy->wspolczynnik_wywazenia == 1 )
		{
			if ( biezacy->prawy && biezacy->prawy->prawy->klucz == klucz )
				rotacja_lewa ( korzen, biezacy->prawy->klucz );
			else
			{
				rotacja_prawa ( korzen, biezacy->prawy->lewy->klucz );
				rotacja_lewa ( korzen, biezacy->prawy->klucz );
			}
		}
		...
		{	
			...
			if ( biezacy->wspolczynnik_wywazenia == -1 )
			{
				if ( biezacy->lewy && biezacy->lewy->lewy->klucz == klucz )
					rotacja_prawa ( korzen, biezacy->lewy->klucz );
				else
				{
					rotacja_lewa ( korzen, biezacy->lewy->prawy->klucz );
					rotacja_prawa ( korzen, biezacy->lewy->klucz );
				}
			}
	...
	/*while(biezacy)
	{
		...
		//bufor_wspolczynnika = biezacy->wspolczynnik_wywazenia;
		...
		if ( biezacy->wspolczynnik_wywazenia == 2 )
		{
			if ( biezacy->prawy->prawy && biezacy->prawy->prawy->klucz == klucz )
			{
				rotacja_lewa ( korzen, biezacy->prawy->klucz );
	...
	/*struct wezel *biezacy, *dalszy, *rotowany;
	int bufor_wspolczynnika;
	int bufor_klucza = klucz;
	int oryginalny_klucz = klucz;
	...
	while(biezacy)
	{
		...
		//if ( abs(bufor_wspolczynnika)>abs(biezacy->wspolczynnik_wywazenia) )
			//break;

		if ( biezacy->wspolczynnik_wywazenia == 2 )
		{
			if ( biezacy->prawy->lewy && biezacy->prawy->lewy->klucz == bufor_klucza )
			{
			...
			if ( biezacy->prawy->prawy && biezacy->prawy->prawy->klucz == bufor_klucza )
				rotacja_lewa(korzen,biezacy->prawy->klucz);
		}

		if ( biezacy && biezacy->wspolczynnik_wywazenia == -2 )
		{
			if ( biezacy->lewy->prawy && biezacy->lewy->prawy->klucz == bufor_klucza )
			{
			...
			if ( biezacy->lewy->lewy && biezacy->lewy->lewy->klucz == bufor_klucza )
			{
